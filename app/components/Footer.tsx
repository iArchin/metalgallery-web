import { toPersianNumber } from "@/app/utils/numbers";
import Image from "next/image";
import Link from "next/link";
import {
  PHONE_KIND_LABELS,
  phoneHref,
  siteAddresses,
  sitePhones,
  type SiteSettings,
} from "@/lib/types";
import ContactIcon, { type ContactIconName } from "@/app/components/ContactIcon";

const supportLinks = [
  { label: "لیست فروشگاه‌ها", href: "/about" },
  { label: "ساعت کاری", href: "/contact" },
  { label: "تماس با ما", href: "/contact" },
  { label: "سیاست بازگشت کالا", href: "/about" },
];

const accountLinks = [
  { label: "صفحه سبد خرید", href: "/cart" },
  { label: "تسویه حساب", href: "/cart" },
  { label: "مقایسه", href: "/products" },
  { label: "لیست علاقه‌مندی‌ها", href: "/wishlist" },
];

const serviceLinks = [
  { label: "مناطق خدمات", href: "/about" },
  { label: "اسباب‌بازی، بازی‌ها", href: "/products" },
  { label: "پیشنهادات خدمات", href: "/news" },
  { label: "جدول قیمت‌گذاری", href: "/products" },
];

export default function Footer({ settings }: { settings: SiteSettings }) {
  // One tile per contact entry, so a shop with three numbers and two branches
  // shows all five rather than the first of each.
  const tiles: {
    key: string;
    icon: ContactIconName;
    label: string;
    value: string;
    href?: string;
    dir?: "ltr" | "rtl";
  }[] = [
    ...sitePhones(settings).map((p) => ({
      key: `phone-${p.id}`,
      icon: p.kind as ContactIconName,
      label: p.label.trim() || PHONE_KIND_LABELS[p.kind],
      value: toPersianNumber(p.value),
      href: phoneHref(p),
      dir: "ltr" as const,
    })),
    {
      key: "email",
      icon: "email" as const,
      label: "ایمیل ما",
      value: settings.email,
      href: settings.email ? `mailto:${settings.email}` : undefined,
      dir: "ltr" as const,
    },
    {
      key: "hours",
      icon: "hours" as const,
      label: "ساعت کاری",
      value: toPersianNumber(settings.workingHours),
    },
    ...siteAddresses(settings).map((a) => ({
      key: `address-${a.id}`,
      icon: "address" as const,
      label: a.label.trim() || "آدرس",
      value: toPersianNumber(a.value),
    })),
  ].filter((t) => t.value);

  return (
    <footer className="bg-surface-2 text-content">
      {/* Top Section - Contact Info */}
      <div className="bg-surface-3 py-8">
        <div className="site-container">
          {/* Column count follows the tile count — a fixed 4 would strand a
              fifth branch on a row of its own. */}
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {tiles.map((t) => {
              const body = (
                <>
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-primary-soft text-primary">
                    <ContactIcon name={t.icon} className="w-6 h-6" />
                  </span>
                  <div className="min-w-0">
                    <div className="text-sm text-content-subtle">{t.label}</div>
                    <div
                      className="font-semibold break-words"
                      dir={t.dir}
                      style={t.dir === "ltr" ? { textAlign: "right" } : undefined}
                    >
                      {t.value}
                    </div>
                  </div>
                </>
              );
              return t.href ? (
                <a
                  key={t.key}
                  href={t.href}
                  className="flex items-center gap-3 rounded-2xl transition-colors hover:text-primary"
                >
                  {body}
                </a>
              ) : (
                <div key={t.key} className="flex items-center gap-3">
                  {body}
                </div>
              );
            })}
          </div>
        </div>
      </div>

      {/* Bottom Section - Links */}
      <div className="py-12 px-4 sm:px-6 bg-dots-fade text-content-subtle">
        <div className="site-container">
          <div className="grid grid-cols-2 gap-6 md:grid-cols-5 md:gap-8 mb-8 items-center">
            {/* About */}
            <div className="col-span-2">
              <Link
                href="/"
                aria-label="متال گالری - صفحه اصلی"
                className="inline-flex items-center gap-2 mb-4"
              >
                <Image
                  src="/images/logo.png"
                  alt="متال گالری"
                  width={250}
                  height={250}
                  className="w-40 sm:w-48 h-auto object-contain dark:brightness-0 dark:invert"
                />
              </Link>
              <p className="text-content-muted mb-4 text-sm">
                {settings.footerAbout}
              </p>
              {/* Hidden when the field is blank: a hardcoded fallback made
                  clearing Instagram in the panel look like it had no effect. */}
              <div className="flex gap-3">
                {settings.socials.instagram && (
                <a
                  href={settings.socials.instagram}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="اینستاگرام"
                  className="w-10 h-10 bg-primary-soft text-primary rounded-full flex items-center justify-center transition-colors hover:bg-primary hover:text-primary-content active:bg-primary active:text-primary-content"
                >
                  <svg
                    className="w-5 h-5"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678c-3.405 0-6.162 2.76-6.162 6.162 0 3.405 2.76 6.162 6.162 6.162 3.405 0 6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 0-1.44-.646-1.44-1.44 0-.794.646-1.439 1.44-1.439.793-.001 1.44.645 1.44 1.439z" />
                  </svg>
                </a>
                )}
              </div>
            </div>

            {/* Customer Support */}
            <div>
              <h3 className="text-lg font-semibold mb-4">پشتیبانی مشتری</h3>
              <ul className="space-y-2 text-content-muted text-sm">
                {supportLinks.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="inline-block py-1 transition-colors hover:text-content active:text-primary"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* My Account */}
            <div>
              <h3 className="text-lg font-semibold mb-4">حساب کاربری من</h3>
              <ul className="space-y-2 text-content-muted text-sm">
                {accountLinks.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="inline-block py-1 transition-colors hover:text-content active:text-primary"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Services */}
            <div>
              <h3 className="text-lg font-semibold mb-4">خدمات</h3>
              <ul className="space-y-2 text-content-muted text-sm">
                {serviceLinks.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="inline-block py-1 transition-colors hover:text-content active:text-primary"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          </div>


        </div>
      </div>

      {/* Copyright Bar */}
      <div className="bg-surface-3 py-4">
        <div className="site-container">
          <div className="flex flex-col items-center gap-2 text-center">
            <p className="text-content-muted text-sm">
              © کپی‌رایت {toPersianNumber("۱۴۰۴")} توسط {settings.siteName}.
            </p>
          </div>
        </div>
      </div>
    </footer>
  );
}
